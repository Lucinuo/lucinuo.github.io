...
- 雲端同步：使用 Google Drive appDataFolder，讓 iPhone / iPad / Mac 同步。

## 資料儲存

方案 B 使用 PWA + Google Drive API。使用 Google 登入後，資料會同步到 Google Drive 的 app 專用隱藏 JSON；未登入時仍會先存在瀏覽器 `localStorage`。
